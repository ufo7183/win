...
function store_filter_activate() {
    ...
    // Flush the rewrite rules to make the 'store' post type accessible
    flush_rewrite_rules();
}
...
if (!function_exists('store_filter_register_post_type')) {
    function store_filter_register_post_type() {
        ...
        store_filter_log('Custom Post Type \'store\' registered.');
    }
...
// 10. Load single store template
if (!function_exists('store_filter_load_single_template')) {
    function store_filter_load_single_template($template) {
        store_filter_log('template_include filter triggered. Original template: ' . $template);
        if (is_singular('store')) {
            store_filter_log('is_singular(\'store\') is TRUE. Checking for template...');
            // Define the path to the template file within the plugin
            $plugin_template = plugin_dir_path( __FILE__ ) . 'single-store.php';
            store_filter_log('Looking for template at: ' . $plugin_template);
            if (file_exists($plugin_template)) {
                store_filter_log('Template found! Overriding with plugin template.');
                return $plugin_template;
            } else {
                store_filter_log('Plugin template NOT found.');
            }
        }
        return $template;
    }
    add_filter('template_include', 'store_filter_load_single_template');
}

// 11. Logging function
if (!function_exists('store_filter_log')) {
    function store_filter_log($message) {
        $logs = get_option('store_filter_logs', array());
        $log_entry = array(
        ...
        update_option('store_filter_logs', $logs, false);
    }
}

// 12. Log Viewer Shortcode
if (!function_exists('store_log_viewer_shortcode')) {
    function store_log_viewer_shortcode() {
        if (!current_user_can('manage_options')) {
            return '<p>您沒有權限查看日誌。</p>';
        }

        // Handle clear log action
        if (isset($_POST['store_filter_clear_log_nonce']) && wp_verify_nonce($_POST['store_filter_clear_log_nonce'], 'store_filter_clear_log_action')) {
            delete_option('store_filter_logs');
        ...
        ob_start();
        ?>
        <div class="store-log-viewer" style="background: #f1f1f1; border: 1px solid #ccc; padding: 15px; margin-top: 20px;">
            <h3 style="margin-top:0;">店家外掛日誌</h3>
            <form method="post" style="margin-bottom: 15px;">
                <?php wp_nonce_field('store_filter_clear_log_action', 'store_filter_clear_log_nonce'); ?>
                <button type="submit" class="button">清除日誌</button>
            </form>
            <pre style="white-space: pre-wrap; background: white; padding: 10px; max-height: 400px; overflow-y: auto;"><?php
            if (empty($logs)) {
                echo '目前沒有日誌紀錄。';
            } else {
    ...
    add_shortcode('store_log_viewer', 'store_log_viewer_shortcode');
}
